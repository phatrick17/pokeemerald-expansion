...
static void OverrideShadowBGPalette(void);
...
static void TryDrawHPBar(void)
{
    OverrideShadowBGPalette();  // set base palette (normal or purple tint)
    OverrideHPBarPalette();     // then apply HP-bar color overrides on top
    if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
...
// Purple tint tone values (0-255 scale used by TintPalette_CustomTone)
// Shadow Pokémon use a purple-tinted summary screen to match their dark aura
#define SHADOW_PAL_R_TONE 160
#define SHADOW_PAL_G_TONE  80
#define SHADOW_PAL_B_TONE 255

static void OverrideShadowBGPalette(void)
{
    // Reload 8 BG palette slots from the original source first, then tint if shadow.
    // This ensures switching from a shadow to a non-shadow mon restores the normal palette,
    // and that OverrideHPBarPalette (called after this) always works from a clean base.
    if (sMonSummaryScreen->summary.isShadow)
    {
        static u16 palBuf[8 * 16];
        CpuCopy16(sSummaryScreen_Pal_BW, palBuf, sizeof(palBuf));
        TintPalette_CustomTone(palBuf, ARRAY_COUNT(palBuf), SHADOW_PAL_R_TONE, SHADOW_PAL_G_TONE, SHADOW_PAL_B_TONE);
        LoadPalette(palBuf, BG_PLTT_ID(0), 8 * PLTT_SIZE_4BPP);
    }
    else
    {
        LoadPalette(sSummaryScreen_Pal_BW, BG_PLTT_ID(0), 8 * PLTT_SIZE_4BPP);
...
// FONT_SHORT is used in places where the lines
