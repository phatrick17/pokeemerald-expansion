...
// Compatibility with expansion 1.14.4 (this menu was written against 1.16.0's
// party refactor). Partner mons in local multi battles occupy gPlayerParty[3..5],
// matching 1.16's gParties[B_TRAINER_PARTNER][0..2] layout exactly.
#ifndef MAX_BATTLE_TRAINERS
#define B_TRAINER_PLAYER  0
#define B_TRAINER_PARTNER 1
#define gParties ((struct Pokemon (*)[MULTI_PARTY_SIZE])gPlayerParty)
#define gPartiesCount (&gPlayerPartyCount)
#define CalculatePartyCount(trainer) CalculatePlayerPartyCount()
#define CalculatePartyCountOfSide(battler) CalculatePlayerPartyCount()
...
#define CreateMonIconIsEgg(species, callback, x, y, subpriority, personality, isEgg) \
    CreateMonIcon((isEgg) ? SPECIES_EGG : (species), callback, x, y, subpriority, personality)

#define AreMultiPartiesFullTeams() FALSE

static inline u8 CalculatePartnerPartyCount(void)
{
    u8 i, count = 0;
...
{
    return (GetMonData(mon, MON_DATA_POKERUS, NULL) & 0xF) != 0;
}

// Both player-side battlers pick from the same party except in multi battles
#define BattlersShareParty(b1, b2) (!(gBattleTypeFlags & BATTLE_TYPE_MULTI))

static inline struct BoxPokemon *GetSelectedBoxMonFromPcOrParty(void)
{
...
#define PARTY_PAL_SHADOW       (1 << 7) // Shadow Pokémon (purple box)
...
static enum CanMoveBeLearned CanTeachMove(struct Pokemon *, u16);
...
static void DisplayPartyPokemonMoves(u8, int, u16, u8, u8);
...
void TryItemHoldFormChange(struct Pokemon *mon, s8 slotId, u8 trainer);
...
static u8 GetPartyBoxPaletteFlags(u8 slot, u8 animNum)
{
    ...
    {
        u16 species = GetMonData(GetPartyMonFromPartyMenuId(slot), MON_DATA_SPECIES);
...
static enum CanMoveBeLearned CanTeachMove(struct Pokemon *mon, u16 move)
{
    if (GetMonData(mon, MON_DATA_IS_EGG))
...
// Shadow Pokémon party box colors (purple), loaded directly instead of from the palette buffer
static const u16 sShadowPalColors[3]          = {RGB(17, 10, 22), RGB(28, 26, 30), RGB(5, 1, 9)};  // body, text, text shadow
static const u16 sShadowSelectionPalColors[3] = {RGB(24, 16, 29), RGB(2, 1, 8), RGB(28, 26, 30)};  // brighter body, inverted text when hovered
...
static void LoadPartyBoxPalette(struct PartyMenuBox *menuBox, u8 palFlags)
{
    ...
    {
        const u16 *colors = (palFlags & PARTY_PAL_SELECTED) ? sShadowSelectionPalColors : sShadowPalColors;
        LoadPalette(&colors[0], sPartyBoxPalOffset1 + palOffset, PLTT_SIZEOF(1));
        LoadPalette(&colors[1], sPartyBoxPalOffsets3[0] + palOffset, PLTT_SIZEOF(1));
        LoadPalette(&colors[2], sPartyBoxPalOffsets3[1] + palOffset, PLTT_SIZEOF(1));
    }
...
static u8 GetPPFontColorIndexForMove(u16 move, u8 currentPP, u8 ppBonuses, int m)
{
...
static void PrintMovePPToWindow(u8 windowId, u8 fontId, u16 move, u8 pp, u8 ppBonuses, int m, int xBase, int y, int areaWidth)
{
...
static void DisplayPartyPokemonMoves(u8 windowId, int m, u16 move, u8 pp, u8 ppBonuses)
{
    const struct PartyMenuMoveBoxInfoRects *info = &sPartyMoveBoxInfoRects[0];
...
static void CreateItemMoveSprite(u8 fromSlot, u8 toSlot, u16 item)
{
    ...
    u16 item2 = GetMonData(&gParties[B_TRAINER_PLAYER][fromSlot], MON_DATA_HELD_ITEM);
    u16 item1 = item;
...
static void ShowMoveSelectWindow(u8 slot)
{
    ...
    u16 move;
    u8 unlockedSlots = GetMonUnlockedMoveSlots(&gParties[B_TRAINER_PLAYER][slot]);
...
static void CB2_ShowSummaryScreenToForgetMove(void)
{
    ShowSelectMovePokemonSummaryScreen(gParties[B_TRAINER_PLAYER], gPartyMenu.slotId, gPlayerPartyCount - 1, CB2_ReturnToPartyMenuWhileLearningMove, gPartyMenu.data1);
}
...
static void DisplayPartyPokemonDataForFormChange(u8 slot)
{
    ...
    targetSpecies = GetFormChangeTargetSpecies(mon, FORM_CHANGE_ITEM_USE, 0);
...
bool32 TryItemUseFormChange(u8 taskId, TaskFunc task)
{
    ...
    if (TryFormChange(gPartyMenu.slotId, B_SIDE_PLAYER, FORM_CHANGE_ITEM_USE))
    {
        gPartyMenuUseExitCallback = TRUE;
        SetWordTaskArg(taskId, tNextFunc, (u32)task);
...
bool32 TryMultichoiceFormChange(u8 taskId)
{
    ...
    if (TryFormChange(gPartyMenu.slotId, B_SIDE_PLAYER, FORM_CHANGE_ITEM_USE_MULTICHOICE))
    {
        gPartyMenuUseExitCallback = TRUE;
...
void TryItemHoldFormChange(struct Pokemon *mon, s8 slotId, u8 trainer)
{
    if (TryFormChange(slotId, B_SIDE_PLAYER, FORM_CHANGE_ITEM_HOLD))
    {
        u32 species = GetMonData(mon, MON_DATA_SPECIES);
